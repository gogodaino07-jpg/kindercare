...
- 예시 E (나이/반 구분이 아예 없는 통신문, 중요): 어떤 유치원/어린이집은 반이 하나뿐이거나, 나이·반 구분 없이 원 전체에 공통으로 통신문을 보냅니다. 통신문 전체를 훑어봤을 때 나이나 반을 언급하는 부분이 단 한 군데도 없다면, 이는 특정 나이/반을 걸러낼 필요가 없는 '전체 공통' 통신문입니다. 대상 아이의 연령·소속 반이 통신문에 직접 등장하지 않는다는 이유로 항목을 제외하지 말고, 언급된 일정·준비물·공지를 전부 이 아이에게 해당하는 것으로 간주해 추출하세요. (반대로 통신문 안에 일부라도 나이/반 구분 표현이 등장한다면 예시 A~D와 [분석 제1원칙]대로 그 구분을 기준으로 필터링하세요.)
- 예시 F (수신인에 아이 이름이 직접 적힌 경우, 나이 불일치보다 최우선): "수신: 별님반 하루(4세) 어머님/아버님께"처럼 통신문에 [대상 아이 정보]의 이름(또는 애칭)이 문자 그대로 적혀 있다면, 이는 이 통신문이 이 아이 앞으로 온 것이라는 가장 확실한 증거입니다. 이땐 문서에 함께 적힌 나이·반이 [대상 아이 정보]의 연령·소속 반과 정확히 일치하지 않더라도(앱에 등록된 나이는 만 나이/연 나이 계산 방식 차이나 사용자의 입력 오차로 유치원 공식 표기와 다를 수 있습니다), 나이·반 불일치를 이유로 절대 제외하지 말고 이름을 근거로 정상적으로 추출하세요. 이름이 같은 다른 아이일 가능성보다, 실제 나이를 등록한 [대상 아이 정보]의 연령이 애초에 부정확할 가능성이 훨씬 높습니다.
- 예시 G (나이/반 구분이 있는 통신문 안에 섞여 있는 원 전체 공통 일정, 중요): 통신문 전체가 나이/반별로 구분되어 있더라도(예시 A~D 상황), 그 안에 특정 나이/반에 묶이지 않고 원 전체 아이들에게 똑같이 적용되는 흐름상의 공통 일정이 함께 섞여 있는 경우가 있습니다 (예: "도서 대출일", "도서 반납일", "우유 신청/집금일", "현장학습 사진 신청 마감일"처럼 특정 연령·반 표기 없이 날짜와 활동명만 적힌 항목). 이런 항목은 어느 나이/반에도 명시적으로 속하지 않는다는 이유로 걸러내지 말고, [분석 제1원칙]의 나이/반 필터링 대상에서 예외로 두어 대상 아이에게도 정상적으로 추출하세요. 판단 기준은 그 항목 자체에 나이/반 표기가 없고, 문맥상 특정 나이/반만을 위한 활동이 아니라 원 운영상 모든 아이에게 공통으로 해당되는 성격(도서관 운영, 우유 급식, 준비물 공동 구매 등)인지입니다 — 반대로 나이/반이 명시된 다른 항목들 사이에 섞여 있다고 해서 자동으로 같은 나이/반 제한이 적용된다고 넘겨짚지 마세요.

[대상 아이 정보]
- 이름: ${child.name ?? '아이'}${child.givenName?.trim() ? ` (애칭: ${child.givenName.trim()})` : ''}
- 연령: ${child.age}세 (이 연령에 해당하는 정보만 추출 — 단, 위 예시 F처럼 통신문에 이 이름이 직접 언급된 경우는 예외)
${
...
2. 필터링 및 우선순위:
   ...
   - 단, 특정 날짜(또는 시작일)가 명시된 공통 안내(예: "OO 교육 주간", "신청 마감일", "이번 주 인성 덕목" 등)는 준비물이 없더라도 생략하지 말고 category: "공지"로 캘린더에 포함하세요. 부모가 놓치면 안 되는 학사 일정이므로, 물건을 챙길 필요가 없다는 이유만으로 완전히 빼지는 마세요. noticeText에 핵심만 짧게 요약하면 됩니다.
   - 기간으로 적힌 공통 안내(예: "9월 28일(월)부터 10월 8일(목)까지 2학기 학부모 상담")도 위 "시작일이 명시된 공통 안내"에 똑같이 해당합니다. 통신문 뒤쪽의 "안내해 드립니다" 같은 번호 목록에 흩어진 날짜 있는 항목들도 하나씩 훑어서, 이 아이에게 해당하는 것은 시작일 하루로 category: "공지"에 빠뜨리지 말고 포함하세요. 단, 재원 학부모가 아닌 외부 대상 안내(예: 신입 원생 모집 입학설명회)는 제외하세요.
   - 날짜가 전혀 없는 순수 공통 안내(예: "다음 주부터 하원 시간이 오후 4시로 변경됩니다", "문의사항은 담임교사 OOO 선생님에게 연락 부탁드립니다"처럼 특정 하루를 콕 집을 수 없는 정책 변경·연락처·협조 요청 등)도, 특정 아이/반에 한정되지 않고 전체 학부모에게 공통으로 해당되는 내용이라면 절대 누락시키지 말고 category: "공지"로 포함하세요. 이런 항목은 date에 [현재 기준 날짜](${todayISO})를 그대로 쓰고, title은 "학급 안내" 등 짧게, noticeText에 핵심 내용을 요약하세요. 다만 특정 반/연령에만 해당한다고 명시된 안내는 [분석 제1원칙]대로 대상 아이 조건에 안 맞으면 제외하세요.
...
5. 요일별 활동/특강 표 처리:
   ...
   - 이런 이벤트의 title은 과목/활동명 그대로 적으세요 (예: "영어", "체육", "하와이체험"). category는 "특별활동" 또는 "행사"로 분류하세요. 단, 칸 내용이 "OO공휴일", "재량휴업일", "휴원"처럼 활동이 아니라 원이 쉬는 날을 뜻한다면 category를 "휴원/방학"으로 분류하세요 — 활동/행사 카테고리에 안 맞는다는 이유로 이런 칸을 건너뛰거나 누락시키지 마세요.
   - 표의 각 날짜 열을 왼쪽부터 하나씩 순서대로 확인하면서 빠짐없이 처리하세요. 날짜 헤더(예: "13일(목)")를 활동 내용과 한 쌍씩 정확히 맞춰서 대응시키고, 절대 옆 칸(하루 전/후)의 날짜에 잘못 배정하지 마세요. 표에 실제 활동명이 적힌 열인데 이벤트를 만들지 않고 건너뛰는 실수를 하지 마세요.
...
6. 색상/강조 표시로 연결된 정보 병합:
   - 통신문 하단 안내 문구(소식란, 협조사항 등)에서 특정 날짜나 활동명이 다른 텍스트와 구별되는 색상·굵기·밑줄(★ 표시 포함)로 강조되어 있고, 그 내용이 위 5번에서 추출한 이벤트와 같은 날짜의 같은 활동을 가리킨다면, 별도 이벤트를 새로 만들지 말고 해당 이벤트에 그 강조 문구 내용을 합쳐서 적으세요.
   - 이때 강조 문구 안에 "빨간 티셔츠", "우비", "장화"처럼 실제로 챙기거나 입어야 할 구체적인 물건이 언급되어 있다면, noticeText/memo에만 적지 말고 반드시 items에도 그 물건을 추가하세요. 같은 이벤트를 다른 자료(월간 행사표 등)에서 "활동복"처럼 포괄적으로만 언급했더라도, 더 구체적인 지시(색깔이 지정된 옷 등)가 따로 있다면 그 구체적인 지시를 반드시 items에 포함하세요 — 포괄적 표현이 있다고 구체적인 지시를 생략하면 안 됩니다.
   - 강조 문구가 가리키는 활동명이 5번에서 표로 추출한 그 날짜의 활동명과 다르다면(예: 표의 칸에는 "영어(직업체험)"라고 적혀 있는데, 소식란 강조 문구는 "동물체험"이라는 별개의 활동을 설명), 이는 병합 대상이 아니라 같은 날짜에 진행되는 또 다른 별개의 활동입니다. 억지로 하나로 합치지 말고 강조 문구의 활동명으로 새 이벤트를 하나 더 만드세요.
