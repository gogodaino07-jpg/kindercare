...
export default function WeatherRegionSettingsScreen() {
  ...
  const { slots, saveToSlot } = useWeatherFavorites();
  ...
  useEffect(() => {
    slots.forEach((slot) => {
      ...
      fetchWeatherPreview(slot.region.latitude, slot.region.longitude).then((result) => {
        setFavoriteTemps((prev) => ({ ...prev, [slot.key]: result?.tempC ?? null }));
      });
    });
  }, [slots]);
  ...
  // 우리집/회사/본가 칸: 비어있으면 지금 지역을 저장, 채워져 있으면 그 지역으로 바로 전환.
      ...
      <SafeAreaView style={styles.safeArea} edges={['bottom']}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.favoritesCard}>
            ...
            {slots.map((slot) => (
              <View key={slot.key} style={styles.favoriteRow}>
                ...
                {slot.region && (
                  <>
                    {favoriteTemps[slot.key] != null && (
                      <Text style={styles.favoriteTemp}>{favoriteTemps[slot.key]}°</Text>
                    )}
